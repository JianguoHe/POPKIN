"""Mass-transfer stability criteria for main-sequence/subgiant donors."""

from popkin.utils import conditional_njit


@conditional_njit
def MT_stability_MS(type_donor, mass1i, mass2i, tbi, mass_accretion_model):
    """Return the critical mass ratio for MS/subgiant donor stability.

    The fitting formulae follow Shao & Li (2014),
    doi:10.1088/0004-637X/796/1/37.

    Args:
        type_donor: Donor stellar type.
        mass1i: Initial donor mass [M_sun].
        mass2i: Initial accretor mass [M_sun].
        tbi: Initial orbital period [days].
        mass_accretion_model: Mass-accretion efficiency scheme.

    Returns:
        Critical mass ratio. Large values mark stable regions in the adopted
        fitting tables; zero marks unstable regions.
    """
    if mass_accretion_model == 'rotation dependent':
        return MT_MS_stability_model_1(type_donor, mass1i, mass2i, tbi)
    elif mass_accretion_model == 'half accretion':
        return MT_MS_stability_model_2(type_donor, mass1i, mass2i, tbi)
    elif mass_accretion_model == 'thermal equilibrium':
        return MT_MS_stability_model_3(type_donor, mass1i, mass2i, tbi)
    else:
        raise ValueError(
            "Unsupported mass_accretion_model. Available options: "
            "'rotation dependent', 'half accretion', 'thermal equilibrium'."
        )


@conditional_njit
def MT_MS_stability_model_1(type_donor, mass1i, mass2i, tbi):
    """Critical mass ratio for the rotation-dependent accretion model."""
    qc = 3
    q1 = mass1i / mass2i
    if type_donor in {0, 1, 2} or (type_donor == 4 and mass1i >= 12.0):
...
@conditional_njit
def MT_MS_stability_model_2(type_donor, mass1i, mass2i, tbi):
    """Critical mass ratio for the half-accretion model."""
    qc = 0
    q1 = mass1i / mass2i
    if type_donor in {0, 1}:
...
@conditional_njit
def MT_MS_stability_model_3(type_donor, mass1i, mass2i, tbi):
    """Critical mass ratio for the thermal-equilibrium accretion model."""
    qc = 0
    q1 = mass1i / mass2i
    if type_donor in {0, 1}:
        if mass1i <= 20.0:

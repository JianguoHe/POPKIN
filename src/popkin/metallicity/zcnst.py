import numpy as np
from popkin.metallicity.zdata import zdata
from popkin.utils import conditional_njit


# Set constants for formulae that depend on metallicity Z.
...
#          4; M below which C/O ignition doesn't occur, Mup.  (unused in this code)
#          5; M above which C ignites in the centre, Mec.     (unused in this code)
#          6; value of log D for M <= Mhef
#          7; value of x for Rgb propto M^(-x)
#          8; value of x for tMS = MAX(tHOOK,x*tBGB)
#          9; Mc at BGB for star with initial mass of Mhef, used to computing Mc at BGB for IM & HM stars
#          10; Mc at HeI for star with initial mass of Mhef, used to computing Mc at HeI for IM & HM stars
#          11; hydrogen abundance.
#          12; helium abundance.
#          13; constant x in rmin = rgb*x**y used by LM CHeB.
#          14; z**0.4 to be used for WD L formula.

# Initial metallicity arrays.
...
def zcnsts_set(self):
    ...
    # zpars[9] and zpars[10] are the core masses at BGB and HeI
    # for a star with initial mass M_HeF.
    # Save current stellar parameters.
    type_old, mass0_old, mass_old = self.type, self.mass0, self.mass
    # Temporarily evaluate a main-sequence star with mass M_HeF.
    self.type, self.mass0, self.mass = 1, self.zpars[2], self.zpars[2]
    self.StellarCal()
    self.zpars[9] = self.lum_to_mc_gb(self.lums[3])
    self.zpars[10] = self.lum_to_mc_gb(self.lums[4])
    # Restore the stellar parameters.
    self.type, self.mass0, self.mass = type_old, mass0_old, mass_old
